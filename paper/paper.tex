...
of being flushed on braching codes, but advanced branch prediction and speculative execution
...
There is no speculative execution, deep pipelines, out-of-order exeuction, or branch prediction
...
very simple cores goes back to the orginial architectual decisions for the GPU.  Graphics applications
...
the different threads, and therefore must always take the penalty of braching.
...
reduced compute power on the GPU~\cite{Daga2011}.  The improvment of the 
...
Decreasing the data trasnsfer time reduces the penalty associated with migrating
...
worse than just the CPU.  The cause for this degredation in performance was likely due to the
overhead of scheduling threads and communciation between those threads, as well as moving data to
...
thing to notice for this benchmark specificially is that the
...
for each kernel launch, it was kept as device-speciifc times. We could not
