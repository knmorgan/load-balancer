...
fused GPU architectures, and their impact on performance, has been investigated.
\end{abstract}
...
such as OpenCL and CUDA allow programmers to utilize the stream processors of the GPU for non-graphics
applications.  However, because of the difficulty in programming for this hardware, most of the
attention is spent on trying to achieve optimal performance on the GPU to leverage the unique hardware
capabilites, while the CPU is left
unused.  By also making use of CPU to process a portion of the overall workload, we are able to achieve higher
...
balancing scheme or otherwise.  Note that there is an obvious minimum for this function the
point where the GPU and CPU workloads are equal.  The above will be faster than just the GPU iff
...
are able to greatly reduce the amount of times when a pipeline flush must occur.  Out-of-order
...
architecture is specifically designed for high-throughput performance.  Large
...
switching.  Newer GPU architectures may contain privative caching schemes, but the majority
of GPUs still on the commodity graphics market do not have this feature.
...
In OpenCL each thread is logically mapped to a so-called ``work-item''.  These work-items are grouped into
...
Devices are grouped into contexts, which contains memory buffers and device specific information.  A queuing system
...
command queue for each context.  Using a separate command queue
for each device will facilitate multiple devices to be used simultaneously, as each command queue
operates on its own separate thread. We also decided to use a separate context for each device, as it would
allow for finer-grained control of data transfers and residency. 
...
\subsection{Discrete GPU Performance}
...
\subsection{Fused APU Performance}
...
Consider Figure \ref{fig:vector_static_fused}, where the overall runtime
...
Additionally, the use of only applications which have a static workload per thread limit the
effectiveness of the dynamic scheduler.  The ability to a priori know the thread workload makes
the static load-balancing scheme more effective than the dynamic scheme.
Using a kernel with a varied workload per thread
would have made it possible for the dynamic scheduler to see improved performance when
compared to the static mapping of data.
